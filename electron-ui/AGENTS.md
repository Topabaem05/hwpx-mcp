...
| NSIS uninstall cleanup | `scripts/installer.nsh` | Removes user app data/log/cache folders on true uninstall |
...
- Keep NSIS uninstall cleanup paths aligned with runtime storage under `app.getPath("userData")`.
- Renderer should call backend through MCP JSON-RPC methods (`initialize`, `tools/list`, `tools/call`) only.
- Keep MCP URL defaults synchronized across `preload.js`, `src/renderer.js`, and `scripts/start-stack.js`.
...
## Related
- Root architecture and commands: `AGENTS.md`
- Backend transport behavior: `hwpx_mcp/server.py`, `hwpx_mcp/config.py`
